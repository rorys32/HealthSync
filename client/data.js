// HealthSync Version 1.3.004 - Data Management
let userDailyData = {};
let supplements = [];
let foods = [];
let token = null;

// Define globally accessible function for date formatting
...
document.addEventListener('DOMContentLoaded', () => {
    const todayKey = getTodayFormatted(); // Define todayKey for UI sync
    resetDailyData();
    login();
    updateDataStatus();
    document.getElementById('stepProgress').value = userDailyData[todayKey]?.steps || 0;
    document.getElementById('waterProgress').value = userDailyData[todayKey]?.water || 0;
});